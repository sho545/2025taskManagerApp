...
public class TasksIdPutApiTest {
  ...
  void tasksIdPut_shouldReturnBadRequest_whenTitleIsEmpty() throws Exception {
    ...
    OffsetDateTime dueDateFromDb = dueDateTimestamp.toInstant().atOffset(ZoneOffset.ofHours(9));
    // DTOオブジェクトを直接作成
    TaskRequest requestDto = new TaskRequest();
  ...
  @DatabaseSetup("/datasets/tasks/two-tasks.xml")
  @ExpectedDatabase(value = "/datasets/tasks/two-tasks.xml", table = "TASKS")
  void tasksIdPutt_shouldReturnBadRequest_whenDueDateIsPast() throws Exception {

    Map<String, Object> taskFromDb = jdbcTemplate.queryForMap("SELECT * FROM tasks WHERE id = 100");
    Timestamp dueDateTimestamp = (Timestamp) taskFromDb.get("DUE_DATE");
    OffsetDateTime dueDateFromDb = dueDateTimestamp.toInstant().atOffset(ZoneOffset.ofHours(9)).minusSeconds(1);
